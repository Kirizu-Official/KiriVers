...
- [ ] Fileset identity, pack JSON, artifact row, storage blob, admin cleanup, and feed enclosure SHA stay one contract (`backend/client-check-contract.md`, `backend/store-adapter-contract.md`)
- [ ] Checked that consumers import shared decoders / projections instead of
      casting payload fields locally
- [ ] Checked that derived state points back to the source event identifier
      (`seq`, `id`, `version`) instead of inventing a second cursor
- [ ] If the feature is a readiness probe, every dependency (DB, object storage, …)
      is checked; do not return success after the first subsystem that happens to work
