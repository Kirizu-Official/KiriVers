...
KiriVers: reuse `CanonicalFilesetSHA256` / `ResolveNeededFileset` and the existing `auto_delta` / native archive builders for preheat and `dynamic_pack`. Do not add a second zip builder on the HTTP path (`backend/client-check-contract.md`, `backend/directory-structure.md`).
